...
#define DEFINE_OEMEC(_num)							       \
	static struct oemec_data oemec_data_##_num;				       \
	static const struct oemec_dev_config oemec_config_##_num = {		       \
		.bus = I2C_DT_SPEC_INST_GET(_num),				       \
		.mtreg = DT_INST_PROP(_num, mtreg),				       \
		.resolution = DT_INST_PROP(_num, resolution)			       \
	};									       \
	SENSOR_DEVICE_DT_INST_DEFINE(_num, oemec_init, NULL,				       \
			      &oemec_data_##_num, &oemec_config_##_num, POST_KERNEL, \
